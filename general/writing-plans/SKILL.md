...
**本技能产出的 plan 头部必须包含以下 caveman 模式指令：**

```
> **面向 AI 代理的工作者：** 使用 caveman 模式与任何交互。简洁指令，减少 token 消耗。技术术语保持精确。使用 executing-plans 执行此计划。
```

这确保执行 agent 在实现时使用压缩格式，减少历史压缩丢失关键信息的风险。
...
**2. 单智能体串行** - 在当前会话中逐个执行任务，批量审查

**选哪种方式？"**

**如果选择多智能体并行：**
- **必需技能：** 使用 `executing-plans`
- 每个任务一个新子智能体 + 两阶段审查
- 任务间自动使用 caveman 模式
