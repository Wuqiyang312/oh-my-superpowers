...
**必需背景：** 在使用此技能前，你必须理解 oh-my-superpowers:test-driven-development。该技能定义了基本的红-绿-重构循环。本技能提供技能专用的测试格式（压力场景、合理化借口表）。
...
**参考类技能（API 文档、语法指南）不在豁免之列**——铁律没有为它们开例外（见 SKILL.md「铁律（与 TDD 相同）」）。变的是测试方式，不是要不要测：测检索与覆盖（能找到正确的信息吗？常见用例都涵盖了吗？），而不是测合规与纪律。分类方法见 [testing-and-hardening-skills.md](testing-and-hardening-skills.md)。
...
不要只是陈述规则——禁止具体的变通方法。把笼统的「删掉它」改成显式枚举被禁变通方法的表述，不留解释空间。完整的 `<Bad>` / `<Good>` 前后对照示例见 [testing-and-hardening-skills.md](testing-and-hardening-skills.md)「明确堵住每个漏洞」。
