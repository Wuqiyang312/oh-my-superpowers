...
**必需背景：** 在使用此技能前，你必须理解 oh-my-superpowers:test-driven-development。该技能定义了基本的红-绿-重构循环。本技能将 TDD 适配到文档编写中。
...
- `description`：第三人称，仅描述何时使用（不是做什么）
  ...
  - 包含具体的症状、场景和上下文
  - **绝不总结技能的流程或工作流**（原因见 [cso-guide.md](cso-guide.md)）
  - 尽量控制在 500 字符以内
...
**未来的 Claude 必须能找到你的技能。** 编写或编辑 `description` / `name`、优化 token 效率，或编写跨技能交叉引用时，加载 [cso-guide.md](cso-guide.md)（必需）。

该指南包含：丰富的描述字段、关键词覆盖、描述性命名、Token 效率目标与字数预算、交叉引用其他技能的正确格式。
...
参见 [graphviz-conventions.dot](graphviz-conventions.dot) 了解 graphviz 样式规则。
...
**必需背景：** oh-my-superpowers:test-driven-development 技能解释了为什么这很重要。相同的原则适用于文档。

## 技能测试与加固

**按技能类型选择测试方法，并堵住合理化漏洞。** 在部署前测试技能、或加固纪律执行类技能时，加载 [testing-and-hardening-skills.md](testing-and-hardening-skills.md)（必需）。

该指南包含：四类技能（纪律执行 / 技术 / 模式 / 参考）各自的测试方法与成功标准、跳过测试的常见借口表，以及经受住合理化考验的手段（明确堵漏、"精神 vs 字面"、借口表、红线列表、CSO 违规症状）。
...
**测试方法论：** 参见 [testing-skills-with-subagents.md](testing-skills-with-subagents.md) 了解完整的测试方法：
- 如何编写压力场景
- 压力类型（时间、沉没成本、权威、疲惫）
- 系统地堵住漏洞
- 元测试技巧
